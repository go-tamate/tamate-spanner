...
func TestSpanner_Get(t *testing.T) {
	spannerTestCase(t, func(ds *SpannerDatasource) error {
		...
		for i, row := range rows {
			...
			if row.Values["DateTest"].Column.Type != driver.ColumnTypeDate {
				t.Fatalf("DateTest ColumnType must be ColumnTypeDate(%d), but actual: %d.", driver.ColumnTypeDate, row.Values["DateTest"].Column.Type)
			}
